...
The same vulnerable code is vendored by [liboqs v0.10.0](https://github.com/open-quantum-safe/liboqs)
([CVE-2024-36405](https://github.com/open-quantum-safe/liboqs/security/advisories/GHSA-f2v9-5498-2vpp)).
...
```bash
make attack-pqcrystal-kyber
./clangover-pqcrystal-kyber
```

### liboqs v0.10.0

Requires CMake and Ninja (`sudo apt install cmake ninja-build`).
...
3. This PoC contains some parameters in `attack.c`. Consider tweaking them for
   your machine. Of special interest are: - `CONFIDENCE_HIGH`: the higher, the more robust - `EVALUATE_EVERY_N_ITERATIONS`: the higher, the more robust - `OUTLIER_THRESHOLD`: can go either way
